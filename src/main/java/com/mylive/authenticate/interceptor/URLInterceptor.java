package com.mylive.authenticate.interceptor;
...
public class URLInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		if (referer != null
				&& !referer.startsWith(GetProperties
						.getDefaultDevpModValue("domainName.domainUrl"))) {
			// response.sendRedirect("http://211.141.87.93:8083/main.do");
			response.sendRedirect(GetProperties
					.getDefaultDevpModValue("domainName.domainMainPage"));
		}
		...
		if (userId == null) {
			...
			userId = CookieUtil.getCookieValue(request,
		...
		if (loginName == null) {
			// HttpServletRequest request = (HttpServletRequest)
			// ctx.get(StrutsStatics.HTTP_REQUEST);
			loginName = CookieUtil.getCookieValue(request,
