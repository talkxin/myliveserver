...
<div id="tabs1">
	<ul>
		...
		<li id="current"><a href="#"><span>增加角色</span></a></li>
	</ul>
</div>
...
<div class="container">
    ...
    <div class="mainbox">
      <form id="dataForm" name="dataForm" action="insertRole.do" method="post"  onsubmit="return checkAll();">
	      <input type="hidden" name="menuId" value="${menuId }">
	      <h3>增加角色信息</h3>
	      <ul class="memlist fixwidth">
	        <li><em>角色名称<span class="red">*</span>：</em>
          		<input type="text" name="roleModel.roleTitle" id="title" value="" class="txt required"  maxlength="25" style="width:200px;" onblur="checkTitle();"/><span id="errorTitle" style="color:red"></span>
        	</li>
	        <li><em>&nbsp; 角色描述：</em>
	          	<textarea name="roleModel.roleDescription" id="remark" class="bigarea min-length-0 max-length-100 textv"  onblur="checkDescription();"></textarea><span id="errorRemark" style="color:red"></span>
	        </li>
			...
			/* 		d.s=function(nodeId){
					...
					d.add(0,-1,'匹克app后台菜单','####','','',true,false,'','','',true);
					${listMenuStr}
					document.write(d);
					//-->
				</script>                       
	      ...
	      <span id="errorMsg" style="color:red">${errorMsg }</span>
	      	...
	      	<a class="btn_blue" href="listRole.do?menuId=${menuId }" target="_self"><span>返回</span></a> 
	      </div>
